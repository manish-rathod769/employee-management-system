...
import { getTime, getTotalHour, getFinalLog } from '../../helpers/attendance';
...
export const addAttendance = async (req, res) => {
  try {
    ...
    // console.log('INITIAL ATTENDANCE');
    ...
    if (req.body.startDate) {
      ...
      attendance.totalMinutes = getTotalHour(
...
export const getAttendance = async (req, res) => {
  try {
    ...
    if (!attendance.toString()) {
      helpers.successResponse(req, res, 'NA', 200);
    } else {
      helpers.successResponse(req, res, attendance, 200);
    }
  } catch (error) {
