...
export const getAllClientController = async (req, res) => {
  try {
    ...
    const allClients = await Client.findAll({
      where: { isArchive: false, [Op.or]: [{ name: { [Op.like]: `%${searchWord}%` } }, { email: { [Op.like]: `%${searchWord}%` } }, { slackId: { [Op.like]: `%${searchWord}%` } }, { city: { [Op.like]: `%${searchWord}%` } }, { state: { [Op.like]: `%${searchWord}%` } }, { country: { [Op.like]: `%${searchWord}%` } }, { organization: { [Op.like]: `%${searchWord}%` } }] }, attributes: ['id', 'name', 'email', 'slackId', 'organization'], order: [[`${sortBy}`, `${sortOrder}`]], offset: (page - 1) * count, limit: count,
    });
